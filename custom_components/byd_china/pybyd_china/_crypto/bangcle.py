...
from pybyd_china._crypto._bangcle_block import (
    BangcleTables,
    decrypt_cbc,
    encrypt_cbc,
)
from pybyd_china._crypto._pkcs7 import add_pkcs7, strip_pkcs7
from pybyd_china.exceptions import BangcleError, BangcleTableLoadError
...
class BangcleCodec:
    ...
    def _load_tables(self) -> BangcleTables:
        ...
        if self._tables_path is not None:
            ...
            tables_path = Path(__file__).resolve().parent.parent / "data" / "bangcle_tables.bin"
            try:
                raw = tables_path.read_bytes()
            except FileNotFoundError as exc:
                raise BangcleTableLoadError(
                    f"bangcle_tables.bin not found at {tables_path}"
                ) from exc
