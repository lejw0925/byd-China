...
from pybyd_china._api import charging as _charging_api
from pybyd_china._api import control as _control_api
from pybyd_china._api import energy as _energy_api
from pybyd_china._api import gps as _gps_api
from pybyd_china._api import hvac as _hvac_api
from pybyd_china._api import latest_config as _latest_config_api
from pybyd_china._api import push_notifications as _push_api
from pybyd_china._api import realtime as _realtime_api
from pybyd_china._api import smart_charging as _smart_api
from pybyd_china._api import vehicle as _vehicle_api
from pybyd_china._api import vehicle_settings as _settings_api
from pybyd_china._api.login import build_login_request, parse_login_response
from pybyd_china._api.login_cn import build_cn_login_request, parse_cn_login_response
from pybyd_china._crypto.bangcle import BangcleCodec
from pybyd_china._crypto.wbsk import WbskCodec
from pybyd_china._transport import EnvelopeCodec
from pybyd_china._crypto.hashing import md5_hex
from pybyd_china._mqtt import BydMqttRuntime, MqttEvent, fetch_mqtt_bootstrap
...
from pybyd_china.models._base import BydBaseModel
from pybyd_china.models.charging import ChargingStatus
from pybyd_china.models.command_gating import evaluate_command_gate
...
from pybyd_china.models.latest_config import VehicleCapabilities, VehicleLatestConfig
...
class BydClient:
    ...
    async def get_car(
        ...
        *,
        ...
        """
        from pybyd_china.car import BydCar  # delayed import to avoid circular dependency
        ...
        *,
        ...
        async def _call() -> VehicleRealtimeData:
            if self._config.is_china_region:
                session = await self.ensure_session()
                transport = self._require_transport()
                decoded, _serial = await _realtime_api.fetch_realtime_endpoint(
                ...
                # Poll once more if the trigger response isn't ready
                serial = decoded.get("requestSerial")
                if isinstance(serial, str) and not VehicleRealtimeData.is_ready_raw(decoded):
                    import asyncio as _asyncio
                    await _asyncio.sleep(poll_interval)
                    decoded2, _serial2 = await _realtime_api.fetch_realtime_endpoint(
                    ...
                    if isinstance(decoded2, dict) and decoded2:
                        decoded = decoded2
                return VehicleRealtimeData.model_validate(decoded)
